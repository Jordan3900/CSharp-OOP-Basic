...
public class DraftManager
{
    ...
    public string RegisterProvider(List<string> arguments)
    {
        try
        {
            ...
            return $"Successfully registred {provider.GetType()} - {provider.Id}";
        }
        catch (Exception ex)
        {
    ...
    public string Day()
    {
        ...
        double energyRequirmentModfier = this.energyRequirementMode[this.mode];
        ...
        var totalEnergyRequired = this.harvesters.Values.Sum(h => h.EnergyRequirement * energyRequirmentModfier);

        if (totalEnergyRequired <= this.totalStoredEnergy)
        {
            summedOreOutput = this.harvesters.Values.Sum(h => h.OreOutput * oreOutputModifier);
        ...
        builder.AppendLine("A day passed").AppendLine($"Energy Provided: {summedEnergyOutput}").AppendLine($"Plumbus ore mine {summedOreOutput}");
    ...
    public string Check(List<string> arguments)
    {
        ...
        if (!harvesters.ContainsKey(id) && !providers.ContainsKey(id))
        {
